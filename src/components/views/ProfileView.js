...
state = {
 ...
 componentDidMount = async() => {
  ...
  render(){
    ...
    if (!this.props.userInfo.uid && this.props.userInfo.awitingApproval) return <Redirect to ='/' />
    return (
              <MDBContainer className = 'profile-container d-flex' fluid>
                  <div className = 'w-25 pr-3'>
                      <img src = {this.state.profile_photoUrl} alt = 'profile' className = 'img-fluid rounded float-left z-depth-1-half' />
                  </div>
                  <div className = 'info-container'>
                      <h4 className = 'left-aligned font-weight-bold p-2'>{this.state.profile_fullName}</h4>
                      <p className = 'left-aligned font-weight-light p-2'>Location: {this.state.profile_city}, {this.state.profile_stateProvince},  {this.state.profile_country}</p>

                      <p className = 'left-aligned p-2' >About me: {this.state.profile_aboutMe}</p>
                      <p className = 'left-aligned p-2'>Account Type: {this.state.profile_userType}</p>
                      <Button
                          size = 'small'
                          color = 'primary'
                          variant = 'contained'
...
//           Account Type: {props.userInfo.userType}
//         </p>
...
// ///NOT BEING USED. DELETE SOON. HAHA.
